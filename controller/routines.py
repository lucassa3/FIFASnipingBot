...
def sell_cards():
    ...
    retry_cmd(goto_transfers, 1, 0, ProgramState.selenium_instance.get_web_driver())
    if current_screen([SellScreen]):
        get_frame().status_text_component.status.configure(text="Going to tradepile")
    retry_cmd(goto_tradepile, 0, 0, ProgramState.selenium_instance.get_web_driver())
    if current_screen([SellScreen]):
        get_frame().status_text_component.status.configure(text="Removing sold cards")
    retry_cmd(remove_sold, 1, 3, ProgramState.selenium_instance.get_web_driver())
    wait_loading(ProgramState.selenium_instance.get_web_driver(), 2)
    if current_screen([SellScreen]):
        get_frame().status_text_component.status.configure(
            text="Selling tradepile cards"
        )
    sell_tradepile_players(ProgramState.selenium_instance.get_web_driver())
    if current_screen([SellScreen]):
...
def farm_bronze_packs_sync_routine():
    while True:
        retry_cmd(goto_store, 1, 0, ProgramState.selenium_instance.get_web_driver())
        retry_cmd(
            goto_bronze_packs, 0, 0, ProgramState.selenium_instance.get_web_driver()
        )
        retry_cmd(
            buy_base_bronze_pack, 0, 0, ProgramState.selenium_instance.get_web_driver()
        )
        retry_cmd(confirm_dialog, 0, 0, ProgramState.selenium_instance.get_web_driver())
        time.sleep(8)
        deal_with_bronze_items(ProgramState.selenium_instance.get_web_driver())
        time.sleep(5)
...
def snipe(
    ...
    retry_cmd(goto_transfers, 0, 0, ProgramState.selenium_instance.get_web_driver())
    tradepile_size = retry_cmd(
        get_tradepile_size, 1, 0, ProgramState.selenium_instance.get_web_driver()
    )
    retry_cmd(
        goto_transfer_search, 0, 0, ProgramState.selenium_instance.get_web_driver()
    )
    ...
    retry_cmd(
        ...
        0,
        0,
        ProgramState.selenium_instance.get_web_driver(),
    )
    ...
    while tradepile_size < max_tradepile_size:
        retry_cmd(confirm_search, 0, 0, ProgramState.selenium_instance.get_web_driver())

        if sell_player == 1:
            result = buy_card(
                ProgramState.selenium_instance.get_web_driver(), sell=False
            )
        else:
            result = buy_card(
                ProgramState.selenium_instance.get_web_driver(), sell=True
            )
        ...
        retry_cmd(
            back_transfer_search, 0, 0, ProgramState.selenium_instance.get_web_driver()
        )
        controller.manage_filter(
            ProgramState.selenium_instance.get_web_driver(), counter, search_filter
        )
        counter += 1