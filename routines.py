...
def sell_cards():
    ...
    if current_screen(SellScreen): get_frame().sell_component.lbl.configure(text="Seling tradepile players")
...
def snipe(
    ...
    while tradepile_size < max_tradepile_size:
        ...
        if result:
            ...
            if current_screen(SnipeScreen): get_frame().snipe_form_component.lbl_total_players.configure(text=f"Total Players Bought : {str(cards_got)}")
            if current_screen(SnipeScreen): get_frame().snipe_form_component.lbl_total_profit.configure(text=f"Total Profit : {str(total_earns-total_spent)}")
